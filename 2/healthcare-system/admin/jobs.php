    ...
    if (!verify_csrf($_POST['csrf'] ?? '')) {
        ...
        if ($title === '') {
            ...
            if ($jobId) {
                ...
                $stmt = $db->prepare(
                    'INSERT INTO jobs (title, department, location, employment_type, summary, description, status, posted_at)
                     VALUES (:title, :department, :location, :employment_type, :summary, :description, :status, :posted_at)'
                );
                $stmt->execute([
                    'title'          => $title,
                    'department'     => $department,
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            <div class="btn-group" role="group">
                                                <a class="btn btn-sm btn-outline-primary" href="<?= url('/admin/jobs.php?edit=' . (int) $job['id']); ?>" title="Edit">
                                                    <i class="bi bi-pencil"></i>
                                                </a>
                                                <?php if (is_admin()): ?>
                                                    <a class="btn btn-sm btn-outline-danger" href="<?= url('/admin/jobs.php?delete=' . (int) $job['id']); ?>" onclick="return confirm('Delete this job?');" title="Delete">
